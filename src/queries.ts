 const prepareValues = (values: string[]) => {
    return values.map(v => {
        if (v.indexOf('\'') !== -1) {
            v = v.replace('\'', '\\\'')
        }
...
export default {
    ...
    createInsert: (tableName: string, columns: string[], values: Array<string>) => {
        if (Array.isArray(columns) && Array.isArray(values)) {
            return `INSERT INTO ${tableName}(${columns.join(",")}) VALUES('${prepareValues(values)}');`
        } return ''
    },
